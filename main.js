const STORAGE_KEY = "binDashboardSettingsV22";
const WEATHER_CACHE_KEY = "binDashboardWeatherCacheV2";
...
function bannerText(settings) {
  if (!settings.ready) return "Set your collection day and week group to begin.";
  ...
  if (daysUntil === 0) {
    return hourNow < 12 ? "Bin day today." : "Bins collected today.";
  }
...
function setLookupStatus(html, cls = "small") {
  const el = document.getElementById("lookupStatus");
  if (!el) return;
  el.className = `lookup-panel ${cls}`;
  el.innerHTML = html;
}
...
async function ensureNotificationsFromUserAction() {
  ...
  if (isIOS() && !isStandalone()) {
    throw new Error("On iPhone or iPad, install this app to the Home Screen first, then open it from the app icon.");
  }
...
async function getVapidPublicKey() {
  const res = await fetchWithTimeout(`${PUSH_BACKEND_URL}/vapid-public-key`, {}, 5000);
  if (!res.ok) throw new Error("Failed to reach push server.");
...
async function subscribeForPush(settings) {
  ...
  const saveRes = await fetchWithTimeout(`${PUSH_BACKEND_URL}/subscribe`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload)
  }, 5000);

  if (!saveRes.ok) throw new Error("Failed to save notification subscription.");

  const welcomeRes = await fetchWithTimeout(`${PUSH_BACKEND_URL}/send-welcome`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
function getWeatherCacheKey(dateIso, lat, lon) {
  return `${dateIso}|${Number(lat).toFixed(3)}|${Number(lon).toFixed(3)}`;
}
...
async function renderWeather(settings, nextDateIso) {
  ...
  try {
    ...
    if (!wx) {
      box.textContent = "No forecast available yet.";
      return;
    }
...
function applySetup() {
  const locality = document.getElementById("localityInput")?.value || "Sunshine Coast";
  const dow = Number(document.getElementById("dowInput")?.value || 1);
  const weekGroup = Number(document.getElementById("weekGroupInput")?.value || 1);
...
function render(settings = loadSettings()) {
  ...
  if (settings.ready) {
    ...
    setLookupStatus(`<span class="warn">Set your collection day and week group, then save.</span>`);
  }
...
function setupManualSetup() {
  ...
  saveBtn.addEventListener("click", async () => {
    ...
    setLookupStatus(`<span class="info">Saving setup…</span>`);
    ...
    try {
      ...
      setLookupStatus(`<span class="success">Setup saved.</span>`);
    } catch (err) {
      setLookupStatus(`<span class="error">${htmlEscape(err.message || "Setup failed.")}</span>`);
    ...
    (async () => {
      try {
        ...
        if (permission === "granted") {
          ...
          saveSettings(result);
          render(result);
          setLookupStatus(`<span class="success">Setup saved. Notifications are on.</span>`);
        } else {
          result.notificationsEnabled = Notification.permission === "granted";
          saveSettings(result);
          render(result);
          setLookupStatus(`<span class="warn">Setup saved. Notifications are not enabled.</span>`);
        }
      } catch (err) {
        console.error(err);
        ...
        saveSettings(result);
        render(result);
        setLookupStatus(`<span class="warn">Setup saved, but notifications could not be enabled right now.</span>`);
      } finally {
        saveBtn.disabled = false;
      }
    })();
